...
def studentpayfee_update(request,id):   
    k = studentnav.objects.filter(parent_category=None) 
    sch = get_object_or_404(Student, mystudent=request.user.id)
    
    if request.method == "POST":
        form_data = request.POST
        
        # Get form inputs and convert them to floats if they are not empty
        term1 = float(form_data['term1']) if form_data['term1'] else 0
        term2 = float(form_data['term2']) if form_data['term2'] else 0
        term3 = float(form_data['term3']) if form_data['term3'] else 0
        amountpaid = float(form_data['amountpaid']) if form_data['amountpaid'] else 0
        
        # Retrieve the fee payment object
        u = get_object_or_404(fee_payment, id=id)
        
        # Calculate the remaining amounts for each term
        if amountpaid >= term1:
            amountpaid -= term1
            term1 = 0
        else:
        ...
        # Save the updated fee payment object
        u.save() 
